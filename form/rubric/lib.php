 ...
 * @return bool
 */
function workshepform_rubric_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('workshepform_rubric', array('id' => $itemid ,'workshepid' => $workshep->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/workshepform_rubric/$filearea/$itemid/$relativepath";
...
class workshep_rubric_strategy implements workshep_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        $this->descriptionopts = array_merge(array('context' => $this->workshep->context), $this->descriptionopts);
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $this->workshep->context, 'workshepform_rubric', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new workshep_edit_rubric_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        if ($DB->record_exists('workshepform_rubric_config', array('workshepid' => $this->workshep->id))) {
            $DB->set_field('workshepform_rubric_config', 'layout', $layout, array('workshepid' => $this->workshep->id));
        } else {
            $record = new stdclass();
            $record->workshepid = $this->workshep->id;
            $record->layout     = $layout;
            $DB->insert_record('workshepform_rubric_config', $record, false);
        }
        ...
        foreach ($data as $record) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('workshepform_rubric', $record);
            } else {
                // exiting field
                $DB->update_record('workshepform_rubric', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $this->workshep->context, 'workshepform_rubric', 'description', $record->id);
            $DB->update_record('workshepform_rubric', $record);
            ...
            foreach ($record->levels as $level) {
                ...
                if (empty($level->id)) {
                    // new field
                    $level->id = $DB->insert_record('workshepform_rubric_levels', $level);
                } else {
                    // exiting field
                    $DB->update_record('workshepform_rubric_levels', $level);
                }
            }
        }
        $DB->delete_records_list('workshepform_rubric_levels', 'id', $deletelevs);
    ...
    public function get_assessment_form(?moodle_url $actionurl=null, $mode='preview', ?stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $this->workshep->context->id, 'workshepform_rubric', 'description',
        ...
        $formclassname = 'workshep_rubric_' . $this->config->layout . '_assessment_form';
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id AS id, d.description AS title, MIN(l.grade) AS min, MAX(l.grade) AS max, 1 AS weight
                  FROM {workshepform_rubric} d
            INNER JOIN {workshepform_rubric_levels} l ON (d.id = l.dimensionid)
    ...
    public static function delete_instance($workshepid) {
        ...
        $dimensions = $DB->get_records('workshepform_rubric', array('workshepid' => $workshepid), '', 'id');
        $DB->delete_records_list('workshepform_rubric_levels', 'dimensionid', array_keys($dimensions));
        $DB->delete_records('workshepform_rubric', array('workshepid' => $workshepid));
        $DB->delete_records('workshepform_rubric_config', array('workshepid' => $workshepid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = "SELECT r.id AS rid, r.sort, r.description, r.descriptionformat,
                  ...
                  FROM {workshepform_rubric} r
             LEFT JOIN {workshepform_rubric_levels} l ON (l.dimensionid = r.id)
                 WHERE r.workshepid = :workshepid
    ...
    protected function load_config() {
        ...
        if (!$config = $DB->get_record('workshepform_rubric_config', array('workshepid' => $this->workshep->id), 'layout')) {
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($this->workshep->context->id, 'workshepform_rubric', 'description', $id);
            }
        }
        $DB->delete_records_list('workshepform_rubric', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link workshep_edit_rubric_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        for ($i = 0; $i < $raw->norepeats; $i++) {
            ...
            $cook[$i]->workshepid           = $this->workshep->id;
            $cook[$i]->sort                 = $i + 1;
