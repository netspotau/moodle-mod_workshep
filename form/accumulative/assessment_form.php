 ...
 * @uses moodleform
 */
class workshep_accumulative_assessment_form extends workshep_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'workshepform_accumulative', $i+1);
            ...
            if ($fields->{'grade__idx_' . $i}) {
                ...
                $mform->addRule(array('grade__idx_' . $i, 'minusone'),
                    get_string('mustchoosegrade', 'workshepform_accumulative'), 'compare', 'gt');
            }
            // comment
