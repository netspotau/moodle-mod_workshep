 ...
 * @return bool
 */
function workshepform_numerrors_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('workshepform_numerrors', array('id' => $itemid ,'workshepid' => $workshep->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/workshepform_numerrors/$filearea/$itemid/$relativepath";
...
class workshep_numerrors_strategy implements workshep_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'workshepform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new workshep_edit_numerrors_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->numerrors; // data to be saved into {workshepform_numerrors}
        $mappings   = $data->mappings;  // data to be saved into {workshepform_numerrors_map}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('workshepform_numerrors', $record);
            } else {
                // exiting field
                $DB->update_record('workshepform_numerrors', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts, $PAGE->context,
                                                      'workshepform_numerrors', 'description', $record->id);
            $DB->update_record('workshepform_numerrors', $record);
        }
        ...
        foreach ($data->mappings as $nonegative => $grade) {
            ...
            if (isset($this->mappings[$nonegative])) {
                $DB->set_field('workshepform_numerrors_map', 'grade', $grade,
                            array('workshepid' => $this->workshep->id, 'nonegative' => $nonegative));
            } else {
                $DB->insert_record('workshepform_numerrors_map',
                            (object)array('workshepid' => $this->workshep->id, 'nonegative' => $nonegative, 'grade' => $grade));
            }
        }
        ...
        $sql = "DELETE FROM {workshepform_numerrors_map}
                      WHERE (($insql nonegative > :maxnonegative) AND (workshepid = :workshepid))";
    ...
    public function get_assessment_form(?moodle_url $actionurl=null, $mode='preview', ?stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'workshepform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new workshep_numerrors_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function get_dimensions_info() {
        ...
        $dimrecords = $DB->get_records('workshepform_numerrors', array('workshepid' => $this->workshep->id), 'sort', 'id,weight,description AS title');
    ...
    public static function delete_instance($workshepid) {
        ...
        $DB->delete_records('workshepform_numerrors', array('workshepid' => $workshepid));
        $DB->delete_records('workshepform_numerrors_map', array('workshepid' => $workshepid));
    }
    ...
    protected function load_mappings() {
        global $DB;
        return $DB->get_records('workshepform_numerrors_map', array('workshepid' => $this->workshep->id), 'nonegative',
                                'nonegative,grade'); // we can use nonegative as key here as it must be unique within workshep
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        $fs         = get_file_storage();
        foreach ($ids as $id) {
            $fs->delete_area_files($PAGE->context->id, 'workshepform_numerrors', 'description', $id);
        }
        $DB->delete_records_list('workshepform_numerrors', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link workshep_edit_numerrors_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->numerrors    = array();          // to be stored in {workshepform_numerrors}
        $cook->mappings     = array();          // to be stored in {workshepform_numerrors_map}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->numerrors[$i]                        = new stdclass();
            $cook->numerrors[$i]->id                    = $raw->{'dimensionid__idx_'.$i};
            $cook->numerrors[$i]->workshepid            = $this->workshep->id;
     ...
     * @return array of filtered records from the table workshep_grades
     */
