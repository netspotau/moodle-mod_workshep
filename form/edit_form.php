 ...
 * @uses moodleform
 */
class workshep_edit_strategy_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform = $this->_form;
        $this->workshep = $this->_customdata['workshep'];
        $this->strategy = $this->_customdata['strategy'];

        $mform->addElement('hidden', 'workshepid', $this->workshep->id);        // workshepid
        $mform->setType('workshepid', PARAM_INT);

        $mform->addElement('hidden', 'strategy', $this->workshep->strategy);    // strategy name
        ...
        $buttonarray[] = $mform->createElement('submit', 'saveandpreview', get_string('saveandpreview', 'workshep'));
