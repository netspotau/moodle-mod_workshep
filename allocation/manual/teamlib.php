...
SQL;
        ...
        $userinfo = $DB->get_records_list('user', 'id', array_keys($participants), '', $userfields);
        ...
        foreach ($submissions as $submission) {
            ...
            if (!empty($allocations[$id])) { // BASE-4672: Ensure there is a valid group to associate team submission.
        ...
        foreach($reviewers as $reviewer) {
            ...
            // BASE-5410: Ensure there is a valid group and reviewer hasn't been allocated to this group yet.
            if (!empty($allocations[$id->group->id]) && empty($allocations[$id->group->id]->reviewedby[$reviewer->reviewerid])) {
