...
class actionbar implements renderable, templatable {
     ...
     * @param \workshep $workshep The workshep object.
     */
    public function __construct(moodle_url $currenturl, \workshep $workshep) {
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        foreach (array_keys($allocators) as $methodid) {
            $selectorname = get_string('pluginname', 'workshepallocation_' . $methodid);
            $menu[$this->workshep->allocation_url($methodid)->out(false)] = $selectorname;
        }
