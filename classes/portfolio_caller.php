...
/**
 * Provides the {@link mod_workshep_portfolio_caller} class.
 *
...
/**
 * Workshep portfolio caller class to integrate with portfolio API.
 *
 * @package   mod_workshep
 * @copyright Loc Nguyen <ndloc1905@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_workshep_portfolio_caller extends portfolio_module_caller_base {

    /** @var workshep The workshep instance where the export is happening. */
    protected $workshep;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('workshep', $this->id, 0, false, MUST_EXIST);
    }
    ...
    public function load_data() {
        ...
        $worksheprecord = $DB->get_record('workshep', ['id' => $this->cm->instance], '*', MUST_EXIST);
        $this->workshep = new workshep($worksheprecord, $this->cm, $this->get('course'));

        $this->submission = $this->workshep->get_submission_by_id($this->submissionid);
        ...
        $canviewallsubmissions = has_capability('mod/workshep:viewallsubmissions', $this->workshep->context);
        $canviewallsubmissions = $canviewallsubmissions && $this->workshep->check_group_membership($this->submission->authorid);

        // Is the user exporting a submission that she/he has peer-assessed?
        $userassessment = $this->workshep->get_assessment_of_submission_by_user($this->submission->id, $USER->id);
        if ($userassessment) {
        ...
        $canviewallassessments = has_capability('mod/workshep:viewallassessments', $this->workshep->context);

        // Load other assessments eventually if the user can see them.
        if ($canviewallassessments or ($ownsubmission and $this->workshep->assessments_available())) {
            foreach ($this->workshep->get_assessments_of_submission($this->submission->id) as $assessment) {
    ...
    public function prepare_package() {
        ...
        $canviewauthornames = has_capability('mod/workshep:viewauthornames', $this->workshep->context, $this->get('user'));

        // Prepare the submission record for rendering.
        $workshepsubmission = $this->workshep->prepare_submission($this->submission, $canviewauthornames);
        ...
        $html .= $this->export_header($workshepsubmission);
        $content = $this->export_content($workshepsubmission);
        // Get rid of the JS relics left by moodleforms.
        $content = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $content);
        $html .= $content;

        if ($writingleap) {
            $leapids[] = $this->export_content_leap2a($leapwriter, $workshepsubmission, $content);
        }
        ...
        if ($writingleap) {
            ...
            $selection = new portfolio_format_leap2a_entry('workshep'.$this->workshep->id,
                get_string('pluginname', 'mod_workshep').': '.s($this->workshep->name), 'selection');
    ...
    protected function add_area_files($filearea, $itemid) {
        ...
        $fs = get_file_storage();
        $areafiles = $fs->get_area_files($this->workshep->context->id, 'mod_workshep', $filearea, $itemid, null, false);
        if ($areafiles) {
     ...
     *
     * @param workshep_submission $workshepsubmission
     * @return string HTML
     */
    protected function export_header(workshep_submission $workshepsubmission) {

        $output = '';
        $output .= html_writer::tag('h2', get_string('pluginname', 'mod_workshep').': '.s($this->workshep->name));
        $output .= html_writer::tag('h3', s($workshepsubmission->title));

        $created = get_string('userdatecreated', 'workshep', userdate($workshepsubmission->timecreated));
        $created = html_writer::tag('span', $created);

        if ($workshepsubmission->timemodified > $workshepsubmission->timecreated) {
            $modified = get_string('userdatemodified', 'workshep', userdate($workshepsubmission->timemodified));
     ...
     * @return string
     */
    protected function export_content(workshep_submission $workshepsubmission) {

        $output = '';

        if (!$workshepsubmission->is_anonymous()) {
            $author = username_load_fields_from_object((object)[], $workshepsubmission, 'author');
            $output .= html_writer::div(get_string('byfullnamewithoutlink', 'mod_workshep', fullname($author)));
        }

        $content = $this->format_exported_text($workshepsubmission->content, $workshepsubmission->contentformat);
        $content = portfolio_rewrite_pluginfile_urls($content, $this->workshep->context->id, 'mod_workshep',
            'submission_content', $workshepsubmission->id, $this->exporter->get('format'));
        $output .= html_writer::div($content);

        $output .= $this->export_files_list('submission_attachment');

        $strategy = $this->workshep->grading_strategy_instance();

        $canviewauthornames = has_capability('mod/workshep:viewauthornames', $this->workshep->context, $this->get('user'));
        $canviewreviewernames = has_capability('mod/workshep:viewreviewernames', $this->workshep->context, $this->get('user'));
        ...
        foreach ($this->assessments as $assessment) {
            ...
            $workshepassessment = $this->workshep->prepare_assessment($assessment, $mform, $options);

            if ($assessment->reviewerid == $this->get('user')->id) {
                $workshepassessment->title = get_string('assessmentbyyourself', 'mod_workshep');
            } else {
                $workshepassessment->title = get_string('assessment', 'mod_workshep');
            }

            $output .= html_writer::empty_tag('hr');
            $output .= $this->export_assessment($workshepassessment);
        }

        if (trim($this->workshep->instructauthors)) {
            $output .= html_writer::tag('h3', get_string('instructauthors', 'mod_workshep'));
            $content = $this->format_exported_text($this->workshep->instructauthors, $this->workshep->instructauthorsformat);
            $content = portfolio_rewrite_pluginfile_urls($content, $this->workshep->context->id, 'mod_workshep',
     ...
     * @return string HTML
     */
    protected function export_assessment(workshep_assessment $assessment) {
        ...
        if ($assessment->reviewer) {
            $output .= html_writer::div(get_string('byfullnamewithoutlink', 'mod_workshep', fullname($assessment->reviewer)));
            $output .= html_writer::empty_tag('br');
        }
    ...
    protected function export_content_leap2a(portfolio_format_leap2a_writer $leapwriter,
            workshep_submission $workshepsubmission, $html) {

        $entry = new portfolio_format_leap2a_entry('workshepsubmission'.$workshepsubmission->id,  s($workshepsubmission->title),
            'resource', $html);
        $entry->published = $workshepsubmission->timecreated;
        $entry->updated = $workshepsubmission->timemodified;
        $entry->author = (object)[
            'id' => $workshepsubmission->authorid,
            'email' => $workshepsubmission->authoremail
        ];
        username_load_fields_from_object($entry->author, $workshepsubmission);
    ...
    public function get_return_url() {
        ...
        $returnurl = new moodle_url('/mod/workshep/submission.php', ['cmid' => $this->cm->id, 'id' => $this->submissionid]);
    ...
    public function check_permissions() {
        return has_capability('mod/workshep:exportsubmissions', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('pluginname', 'mod_workshep');
    }

    /**
     * What export formats the workshep generally supports.
     *
