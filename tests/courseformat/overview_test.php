...
/**
 * Tests for Workshep overview integration.
 *
...
final class overview_test extends \advanced_testcase {
    ...
    public function test_get_grade_item_names(
        ...
        // Workshep stores in number 1 the assessment grade and in number 0 the submission grade.
        $gradeitems = [];
        foreach ($workshepitems as $workshepitem) {
            if ($workshepitem->itemnumber == 0) {
                $gradeitems['submission'] = $workshepitem;
            } else {
                $gradeitems['assessment'] = $workshepitem;
            }
        }
        ...
        $this->assertEquals(get_string('overview_submission_grade', 'mod_workshep'), $items[0]->get_name());
        $this->assertEquals($expectedsubmissions, $items[0]->get_value());

        $this->assertEquals(get_string('overview_assessment_grade', 'mod_workshep'), $items[1]->get_name());
    ...
    public function test_get_extra_phase_overview(string $user, int $currentphase): void {
        ...
        $this->assertEquals(get_string('phase', 'mod_workshep'), $item->get_name());
    ...
    public static function data_provider_get_extra_phase_overview(): \Generator {
        global $CFG;
        require_once($CFG->dirroot . '/mod/workshep/locallib.php'); // BASE-5518: Ensure locallib is loaded.
        yield 'teacher setup phase' => [
            'user' => 'teacher',
            'currentphase' => \workshep::PHASE_SETUP,
        ];
        ...
        yield 'teacher submission phase' => [
            'user' => 'teacher',
            'currentphase' => \workshep::PHASE_SUBMISSION,
        ];
        ...
        yield 'teacher assessment phase' => [
            'user' => 'teacher',
            'currentphase' => \workshep::PHASE_ASSESSMENT,
        ];
        ...
        yield 'teacher evaluation phase' => [
            'user' => 'teacher',
            'currentphase' => \workshep::PHASE_EVALUATION,
        ];
        ...
        yield 'teacher closed phase' => [
            'user' => 'teacher',
            'currentphase' => \workshep::PHASE_CLOSED,
        ];
    ...
    public function test_get_extra_deadline_overview(
        ...
        $this->assertEquals(get_string('deadline', 'mod_workshep'), $item->get_name());
    ...
    public function test_get_extra_submissions_overview(
        ...
        if ($hasstudentactivity) {
            ...
            // BASE-5518: Removed: Breaks 'Add unique index to assessments table to improve assesment allocation.'
        }
        ...
        $this->assertEquals(get_string('submissions', 'mod_workshep'), $itemsubmissions->get_name());
        $this->assertEquals($expected['submissions'], $itemsubmissions->get_value());

        $this->assertEquals(get_string('assessments', 'mod_workshep'), $itemassessment->get_name());
    ...
    public static function data_provider_get_extra_submissions_overview(): \Generator {
        ...
        yield 'teacher setup phase with activity' => [
            ...
            'expectnull' => false,
            'expected' => ['submissions' => 2, 'assessments' => 2], // BASE-5518.
        ];
    ...
    public function test_get_actions_overview(
        ...
        $course = $this->getDataGenerator()->create_course();
        $currentuser = $this->getDataGenerator()->create_and_enrol($course, $role);
        $activity = $this->getDataGenerator()->create_module('workshep', ['course' => $course->id]);
