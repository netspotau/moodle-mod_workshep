...
require_once($CFG->dirroot . '/mod/workshep/classes/portfolio_caller.php');

/**
 * Unit tests for mod_workshep_portfolio_caller class
 *
 * @package    mod_workshep
 * @copyright  2016 An Pham Van <an.phamvan@harveynash.vn>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class mod_workshep_porfolio_caller_testcase extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $workshep = $this->getDataGenerator()->create_module('workshep', ['course' => $course]);
        $this->cm = get_coursemodule_from_instance('workshep', $workshep->id, $course->id, false, MUST_EXIST);
        $this->workshep = new testable_workshep($workshep, $this->cm, $course);
    }
    ...
    public function test_load_data() {
        ...
        $this->getDataGenerator()->enrol_user($student1->id, $this->workshep->course->id);
        $this->getDataGenerator()->enrol_user($student2->id, $this->workshep->course->id);
    ...
    public function test_check_permissions_exportownsubmissionassessment() {
        ...
        $this->getDataGenerator()->enrol_user($student1->id, $this->workshep->course->id, $roleids['student']);
        $this->getDataGenerator()->enrol_user($student2->id, $this->workshep->course->id, $roleids['student']);
        $workshepgenerator = $this->getDataGenerator()->get_plugin_generator('mod_workshep');
        $subid1 = $workshepgenerator->create_submission($this->workshep->id, $student1->id);
        $asid1 = $workshepgenerator->create_assessment($subid1, $student2->id);
        ...
        role_change_permission($roleids['student'], $context, 'mod/workshep:exportsubmissions', CAP_ALLOW);
